...
#include "indirect_mesh_shader_pipeline.h"
...
namespace Renderer
{
IndirectMeshShaderPipeline::IndirectMeshShaderPipeline() = default;

IndirectMeshShaderPipeline::~IndirectMeshShaderPipeline() = default;

IndirectMeshShaderPipeline::IndirectMeshShaderPipeline(VulkanContext* context, VkDescriptorSetLayout bindlessDescriptorSet) : context(context)
{
    VkPushConstantRange renderPushConstantRange{};
    renderPushConstantRange.offset = 0;
    renderPushConstantRange.size = sizeof(IndirectMeshShaderPushConstants);
    ...
    if (!VkHelpers::LoadShaderModule("shaders\\meshShaderIndirect_task.spv", context->device, &taskShader)) {
        throw std::runtime_error("Error when building the vertex shader (meshShader_task.spv)");
    }
    if (!VkHelpers::LoadShaderModule("shaders\\meshShaderIndirect_mesh.spv", context->device, &meshShader)) {
        throw std::runtime_error("Error when building the fragment shader (meshShader_mesh.spv)");
    }
    if (!VkHelpers::LoadShaderModule("shaders\\meshShaderIndirect_fragment.spv", context->device, &fragmentShader)) {
...
IndirectMeshShaderPipeline::IndirectMeshShaderPipeline(IndirectMeshShaderPipeline&& other) noexcept
{
...
IndirectMeshShaderPipeline& IndirectMeshShaderPipeline::operator=(IndirectMeshShaderPipeline&& other) noexcept
{
    if (this != &other) {
        pipelineLayout = std::move(other.pipelineLayout);
        pipeline = std::move(other.pipeline);
