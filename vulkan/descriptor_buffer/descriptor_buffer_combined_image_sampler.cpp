...
bool DescriptorBufferCombinedImageSampler::UpdateDescriptorSet(std::span<VkDescriptorImageInfo> imageInfos, int32_t descriptorSetIndex)
{
    ...
    for (int32_t i = 0; i < imageInfos.size(); i++) {
        ...
        char* bufferPtr = static_cast<char*>(buffer.allocationInfo.pMappedData) + offset + i * combinedImageSamplerSize;
...
bool DescriptorBufferCombinedImageSampler::UpdateDescriptor(const VkDescriptorImageInfo& imageInfo, int32_t descriptorSetIndex, int32_t bindingIndex)
{
    if (descriptorSetIndex < 0 || descriptorSetIndex >= maxDescriptorSets) {
        LOG_ERROR("Invalid descriptor set index: {}", descriptorSetIndex);
        return false;
    ...
    const size_t combinedImageSamplerSize = VulkanContext::deviceInfo.descriptorBufferProps.combinedImageSamplerDescriptorSize;
    char* bufferPtr = static_cast<char*>(buffer.allocationInfo.pMappedData) + offset + bindingIndex * combinedImageSamplerSize;
    vkGetDescriptorEXT(context->device, &descriptorGetInfo, combinedImageSamplerSize, bufferPtr);
