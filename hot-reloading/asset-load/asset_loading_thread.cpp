...
void AssetLoadingThread::Initialize(Renderer::VulkanContext* context_, Renderer::ResourceManager* resourceManager_)
{
    context = context_;
    ...
    VkCommandBufferAllocateInfo cmdInfo = Renderer::VkHelpers::CommandBufferAllocateInfo(4, commandPool);
    std::array<VkCommandBuffer, Renderer::ASSET_LOAD_ASYNC_COUNT> commandBuffers{};
    ...
    for (int32_t i = 0; i < uploadStagingDatas.size(); ++i) {
        ...
        uploadStagingDatas[i].stagingBuffer = Renderer::VkResources::CreateAllocatedStagingBuffer(context, Renderer::STAGING_BUFFER_SIZE);
    }
    ...
    while (!bShouldExit.load()) {
        ...
        for (int i = static_cast<int>(modelsInProgress.size()) - 1; i >= 0; --i) {
            ...
            if (modelEntry->uploadStagingHandles.empty()) {
                ...
                completeQueue.push({inProgress.modelEntryHandle});
                modelsInProgress.erase(modelsInProgress.begin() + i);
                didWork = true;
            }
        }
        ...
        if (uploadStagingHandleAllocator.IsAnyFree()) {
            ...
            while (requestQueue.pop(loadRequest)) {
                LoadGltf(loadRequest.modelEntryHandle, loadRequest.path);
                modelsInProgress.push_back({loadRequest.modelEntryHandle});
                didWork = true;
            }
        }
...
RequestLoad AssetLoadingThread::RequestLoad(const std::filesystem::path& path)
{
    if (pathToHandle.contains(path)) {
        ModelEntryHandle modelHandle = pathToHandle.at(path);
        ModelEntry* model = models.Get(modelHandle);
        if (!model) {
            LOG_ERROR("Model path found in pathToHandle but missing from model free list");
            return {false, ModelEntryHandle::Invalid};
        }

        model->refCount++;
        if (model->state.load(std::memory_order::memory_order_acquire) == ModelEntry::State::Ready) {
            return {true, modelHandle};
        }
        else {
    ...
    ModelEntryHandle newModelEntryHandle = models.Add();
    pathToHandle[path] = newModelEntryHandle;
    requestQueue.push({newModelEntryHandle, path});
    ModelEntry* newModel = models.Get(newModelEntryHandle);
    newModel->refCount = 1;
    return {false, newModelEntryHandle};
}

void AssetLoadingThread::ResolveLoads(RingBuffer<ModelEntryHandle, MAX_LOADED_MODEL_RING_BUFFER>& loadedModels, std::vector<VkBufferMemoryBarrier2>& bufferBarriers, std::vector<VkImageMemoryBarrier2>& imageBarriers)
{
    AssetLoadComplete loadComplete{};
    while (completeQueue.pop(loadComplete)) {
        loadedModels.Push(loadComplete.modelEntryHandle);
        if (ModelEntry* model = models.Get(loadComplete.modelEntryHandle)) {
            bufferBarriers.insert(bufferBarriers.end(), model->modelAcquires.bufferAcquireOps.begin(), model->modelAcquires.bufferAcquireOps.end());
            imageBarriers.insert(imageBarriers.end(), model->modelAcquires.imageAcquireOps.begin(), model->modelAcquires.imageAcquireOps.end());
            model->modelAcquires.Clear();
        }
    }
}
...
Renderer::ModelData* AssetLoadingThread::GetModelData(ModelEntryHandle handle)
{
...
ModelEntryHandle AssetLoadingThread::LoadGltf(ModelEntryHandle newModelHandle, const std::filesystem::path& path)
{
    ...
    for (const fastgltf::Sampler& gltfSampler : gltf.samplers) {
        ...
        samplerInfo.magFilter = Renderer::ModelLoadUtils::ExtractFilter(gltfSampler.magFilter.value_or(fastgltf::Filter::Nearest));
        samplerInfo.minFilter = Renderer::ModelLoadUtils::ExtractFilter(gltfSampler.minFilter.value_or(fastgltf::Filter::Nearest));
        samplerInfo.mipmapMode = Renderer::ModelLoadUtils::ExtractMipmapMode(gltfSampler.minFilter.value_or(fastgltf::Filter::Linear));
        newModel->data.samplers.push_back(Renderer::VkResources::CreateSampler(context, samplerInfo));
    }
    ...
    for (const fastgltf::Material& gltfMaterial : gltf.materials) {
        Renderer::MaterialProperties material = Renderer::ModelLoadUtils::ExtractMaterial(gltf, gltfMaterial);
        materials.push_back(material);
    }

    std::vector<Renderer::Vertex> primitiveVertices{};
    std::vector<uint32_t> primitiveIndices{};
    std::vector<Renderer::Primitive> primitives{};
    ...
    for (fastgltf::Mesh& mesh : gltf.meshes) {
        ...
        for (fastgltf::Primitive& p : mesh.primitives) {
            Renderer::Primitive primitiveData{};

            if (p.materialIndex.has_value()) {
                primitiveData.materialIndex = p.materialIndex.value();
                primitiveData.bHasTransparent = (static_cast<Renderer::MaterialType>(materials[primitiveData.materialIndex].alphaProperties.y) == Renderer::MaterialType::TRANSPARENT_);
            }
            ...
            primitiveData.boundingSphere = Renderer::ModelLoadUtils::GenerateBoundingSphere(primitiveVertices);
    ...
    for (fastgltf::Animation& gltfAnim : gltf.animations) {
        ...
        for (fastgltf::AnimationSampler& animSampler : gltfAnim.samplers) {
            Renderer::AnimationSampler sampler;
            ...
            switch (animSampler.interpolation) {
                case fastgltf::AnimationInterpolation::Linear:
                    sampler.interpolation = Renderer::AnimationSampler::Interpolation::Linear;
                    break;
                case fastgltf::AnimationInterpolation::Step:
                    sampler.interpolation = Renderer::AnimationSampler::Interpolation::Step;
                    break;
                case fastgltf::AnimationInterpolation::CubicSpline:
                    sampler.interpolation = Renderer::AnimationSampler::Interpolation::CubicSpline;
                    break;
            }
        ...
        for (auto& gltfChannel : gltfAnim.channels) {
            Renderer::AnimationChannel channel{};
            ...
            switch (gltfChannel.path) {
                case fastgltf::AnimationPath::Translation:
                    channel.targetPath = Renderer::AnimationChannel::TargetPath::Translation;
                    break;
                case fastgltf::AnimationPath::Rotation:
                    channel.targetPath = Renderer::AnimationChannel::TargetPath::Rotation;
                    break;
                case fastgltf::AnimationPath::Scale:
                    channel.targetPath = Renderer::AnimationChannel::TargetPath::Scale;
                    break;
                case fastgltf::AnimationPath::Weights:
                    channel.targetPath = Renderer::AnimationChannel::TargetPath::Weights;
                    break;
            }
    ...
    size_t sizeVertices = vertices.size() * sizeof(Renderer::Vertex);
    size_t sizeIndices = indices.size() * sizeof(uint32_t);
    size_t sizeMaterials = materials.size() * sizeof(Renderer::MaterialProperties);
    size_t sizePrimitives = primitives.size() * sizeof(Renderer::Primitive);

    if (sizeVertices > Renderer::STAGING_BUFFER_SIZE) {
    ...
    if (sizePrimitives > Renderer::STAGING_BUFFER_SIZE) {
    ...
    for (const Renderer::AllocatedImage& image : newModel->data.images) {
        VkImageViewCreateInfo imageViewCreateInfo = Renderer::VkHelpers::ImageViewCreateInfo(image.handle, image.format, VK_IMAGE_ASPECT_COLOR_BIT);
        newModel->data.imageViews.push_back(Renderer::VkResources::CreateImageView(context, imageViewCreateInfo));
    }
    ...
    for (Renderer::MaterialProperties& material : materials) {
    ...
    // Vertices
    {
        ...
        VkBufferMemoryBarrier2 vertexBarrier = Renderer::VkHelpers::BufferMemoryBarrier(
        ...
        newModel->modelAcquires.bufferAcquireOps.push_back(Renderer::VkHelpers::BufferMemoryBarrier(
                resourceManager->megaVertexBuffer.handle,
    ...
    // Indices
    {
        ...
        VkBufferMemoryBarrier2 indexBarrier = Renderer::VkHelpers::BufferMemoryBarrier(
    ...
    // Materials
    {
        ...
        VkBufferMemoryBarrier2 materialBarrier = Renderer::VkHelpers::BufferMemoryBarrier(
    ...
    uint32_t vertexOffsetCount = newModel->data.vertexAllocation.offset / sizeof(Renderer::Vertex);
    uint32_t materialOffsetCount = newModel->data.materialAllocation.offset / sizeof(Renderer::MaterialProperties);
    ...
    // Primitives
    {
        ...
        uint32_t primitiveOffsetCount = newModel->data.primitiveAllocation.offset / sizeof(Renderer::Primitive);
        ...
        VkBufferMemoryBarrier2 primitiveBarrier = Renderer::VkHelpers::BufferMemoryBarrier(
...
void AssetLoadingThread::UploadTexture(const Renderer::VulkanContext* context, ModelEntry* newModel, const UploadStaging* currentUploadStaging, Renderer::AllocatedImage& image,
                                       ...
                                       const uint32_t stagingOffset)
{
    ...
    newModel->modelAcquires.imageAcquireOps.push_back(
        Renderer::VkHelpers::ImageMemoryBarrier(
            image.handle,
            Renderer::VkHelpers::SubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT),
            VK_PIPELINE_STAGE_2_NONE, VK_ACCESS_2_NONE, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
...
void AssetLoadingThread::CreateDefaultResources()
{
    ...
    VkImageCreateInfo whiteImageCreateInfo = Renderer::VkHelpers::ImageCreateInfo(
...
void AssetLoadingThread::StartUploadStaging(const UploadStaging& uploadStaging)
{
    ...
    VkCommandBufferSubmitInfo cmdSubmitInfo = Renderer::VkHelpers::CommandBufferSubmitInfo(uploadStaging.commandBuffer);
                                        ...
                                        const std::filesystem::path& parentFolder)
{
    ...
    for (const fastgltf::Image& gltfImage : asset.images) {
        ...
        if (stbiData) {
            ...
            if (size > Renderer::STAGING_BUFFER_SIZE) {
                ...
                stbiData = nullptr;
                newModelEntry->data.images.push_back(Renderer::AllocatedImage{}); // VK_NULL_HANDLE
                continue;
            }
            ...
            newImage = Renderer::VkResources::CreateAllocatedImage(context, imageCreateInfo);
...
void AssetLoadingThread::TopologicalSortNodes(std::vector<Renderer::Node>& nodes, std::vector<uint32_t>& oldToNew)
{
